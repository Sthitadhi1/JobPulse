...
class OracleTaleoATSConnector(BaseConnector):
    name: str = "Oracle Taleo"
    source_type: str = "ATS"
    version: str = "2.0.0"
        ...
        async with httpx.AsyncClient(timeout=6.0, follow_redirects=True) as client:
            try:
                ...
                if res.status_code == 200:
                    links = re.findall(r'href=["\']([^"\']*(?:jobdetail|job|position)[^"\']*)["\']', res.text, re.IGNORECASE)
                    for link in links:
        ...
        async with httpx.AsyncClient(timeout=6.0, follow_redirects=True) as client:
            try:
                ...
                if res.status_code == 200:
                    ...
                    return {
                        "external_job_id": f"taleo-{board_token}-{hash(job_url)}",
                        "title": title,
                        "company": company_name,
                        "department": None,
                        ...
                        "salary": None,
                        "description": f"Oracle Taleo posting for {title} at {company_name}",
                        "source": self.name,
