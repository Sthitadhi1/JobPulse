...
class JobviteATSConnector(BaseConnector):
    name: str = "Jobvite"
    source_type: str = "ATS"
    version: str = "2.0.0"
    ...
    async def stage1_discover(self, company: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        careers_url = company.get("careers_url", f"https://jobs.jobvite.com/{board_token}")
        discovered = []
        ...
        async with httpx.AsyncClient(timeout=6.0, follow_redirects=True) as client:
            try:
                ...
                if res.status_code == 200:
                    links = re.findall(r'href=["\']([^"\']*(?:/job/|/position/)[^"\']*)["\']', res.text, re.IGNORECASE)
                    for link in links:
    ...
    async def stage2_extract(self, discovery_item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient(timeout=6.0, follow_redirects=True) as client:
            try:
                ...
                if res.status_code == 200:
                    title_match = re.search(r'<h2[^>]*class="jv-header"[^>]*>(.*?)</h2>|<h1[^>]*>(.*?)</h1>', res.text, re.IGNORECASE | re.DOTALL)
                    raw_title = title_match.group(1) or title_match.group(2) if title_match else None
                    ...
                    return {
                        "external_job_id": f"jv-{board_token}-{hash(job_url)}",
                        "title": title,
                        "company": company_name,
                        "department": None,
                        ...
                        "salary": None,
                        "description": f"Jobvite opportunity for {title} at {company_name}",
                        "source": self.name,
